...
t_int *lpc_perform(t_int *w) 
{
  ...
  //x->vectorBuffer = (t_float *) getzbytes( nfft02 * sizeof(t_float));
  ...
  while (n--) 
	...
	if ((2*nfft*log2(nfft)+nfft) > length*p) { //NOTE: change this to update only when order is changed!
	  ...
	  for(i=0;i<nfft02;i++)
	  {
	    x->split.fReal[i] = winFrameBuffer[i*2];
	    x->split.fImag[i] = winFrameBuffer[i*2+1];
	  ...
	  mayer_fft(nfft02,x->split.fReal,x->split.fImag);
	  ...
	  mayer_rfft(nfft02,x->split.fReal,x->split.fImag);
	...
	for (i = 1; i <= p; i++) {
	  ...
	  if (i != p) {
	    x->l_W[i] = gsl_vector_float_get(x->lR,i+1);
	    for (j = 1; j <= i; j++) {
	      x->l_W[i] -= x->l_A[j][i] * gsl_vector_float_get(x->lR,i-j+1);
	    }
	  }
	}
	...
	x->l_G = sqrt(x->l_E[p]);
	for (i=0; i < p; i++) {
	  x->l_outCoeff_buff[i] = (t_float)(x->l_A[i+1][p]);
	  x->l_outParcor_buff[i] = (t_float)(x->l_K[i+1]);
	  //#ifdef DEBUG
...
void lpc_init(t_lpc *x)
{
	...
	if (x->l_fs < (t_float)(x->l_frame_rate * x->l_order)) {
	  x->l_frame_rate = (long)(x->l_fs / (t_float)x->l_order);
		error("mbc.lpc~: framerate * order must be less than or equal to sampling rate.  framerate has been changed to %d", x->l_frame_rate);
	}
	...
	x->l_order_max = (int)(x->l_fs / (t_float)x->l_frame_rate);
	x->l_frame_size = x->l_hop_size * 2;
	x->l_inframe_idx = x->l_hop_size;
	x->l_outframe_idx = 0;
	x->l_log2nfft = (int) NLOG2(x->l_frame_size+MAX_ORDER+1);
	...
	//allocate memory
	x->l_frame_buff = (t_float *) getzbytes( x->l_frame_size * sizeof(t_float));
	x->l_winframe_buff = (t_float *) getzbytes( x->l_maxnfft * sizeof(t_float));
	x->l_outCoeff_buff = (t_float *) getzbytes(MAX_ORDER * sizeof(t_float));
	x->l_outParcor_buff = (t_float *) getzbytes(MAX_ORDER * sizeof(t_float));
	x->l_outError_buff = (t_float *) getzbytes( x->l_hop_size * sizeof(t_float));
	x->l_win = (t_float *) getzbytes( x->l_frame_size * sizeof(t_float));
	x->l_R = (t_float *) getzbytes( x->l_maxnfft * sizeof(t_float));
	x->l_W = (double *) getzbytes( (MAX_ORDER + 1) * sizeof(double));
	x->l_E = (double *) getzbytes( (MAX_ORDER + 1) * sizeof(double));
	x->l_K = (double *) getzbytes( (MAX_ORDER + 1) * sizeof(double));
	x->l_A = (double **) getzbytes( (MAX_ORDER + 1) * sizeof(double*));
...
void lpc_window(t_lpc *x, t_symbol *s)
{
  if(*s == gensym("bartlett")) lpc_bartlett(x);
  else if(*s == gensym("hanning")) lpc_hanning(x);
  else lpc_hamming(x);
}
