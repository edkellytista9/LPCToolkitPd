...
inline void allpole_highOrdFilter(t_allpole* x, int N, int order, t_float* in, t_float* out)
{
	...
	for (n=N; n!=0; n--) 
	{ 
	  //gsl_vector_memcpy(x->a_tempvec,x->a_a);
	  //gsl_vector_mul(x->a_tempvec,x->a_y);
...
#elif defined(MBC_GSL) // we need to define this, and also import gsl lib
inline void allpole_highOrdFilter(t_allpole* x, int N, int order, t_float* in, t_float* out)
{
	int n, i;
	t_mbcfloat val;
	...
	for (n=N; n!=0; n--) 
	{
	  //probably need some size checks on vector sizes before we do this
	  gsl_vector_memcpy(x->a_tempvec,x->a_a);
	  gsl_vector_mul(x->a_tempvec,x->a_y);
	  //vDSP_vmulD(x->a_a,1,x->a_y,1,x->a_tempVec,1,order);
	  vDSP_sveD(x->a_tempVec,1,&val,order); // do we need to just for() to sum the vectors?
		val =  x->a_G * (t_mbcfloat)(*(in++)) + val;
		for (i=order-1; i>0; i--) x->a_y[i] = x->a_y[i-1];
		x->a_y[0] = val;		
